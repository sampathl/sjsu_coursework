...
void compute_prime (bool Prime_id[], int size); 
...
void print_prime(bool Prime_id[], int size);
...
int main()
{
    bool Prime_id[MAX_NUMBER] ; 	// Array of size MAX_NUMBER.
    for (int i=0; i<MAX_NUMBER; i++) 	// Loop for array priliminary true assignment.
    {
        Prime_id[i]= true;		
    }
    
    compute_prime (Prime_id,MAX_NUMBER); 	// Function call to compute prime numbers.
    print_prime(Prime_id,MAX_NUMBER);		//function call to print the prime numbers.
}
...
void compute_prime(bool Prime_id[],int size)
{
    ...
    {
        if (Prime_id[i] == true)		// Check to see if the number is composite or not. 
        {
            for (int j=(i*i); j<size;j=j+i)
            {
                Prime_id[j]=false;		// Composite number identification assignment.
            }
        }
    }
}
...
void print_prime(bool Prime_id[], int size)
{
    ...
    for (int i=2; i<size; i++)		// Loop for array check.
    {
        ...
        if (Prime_id[i]==true)			// Prime number check using array values.
        {
