...
int main()
{
	...
	do
    {
		...
		// Near plan.
		Plan_no=1;
		Output_header(Plan_no, plant_count );
		Cumi_Step_units_near=Plan_Near(UNITS_PER_CAN, plant_count, Plan_no);

		// Plan Far
		Plan_no=2;
		Output_header(Plan_no, plant_count);
		Cumi_Step_units_far=Plan_Far(UNITS_PER_CAN, plant_count, Plan_no);
...
void Output_header(int a, int plant_count)
{
	if (a==1)
	{
		 cout << endl;
...
void Output_details(int a, int b, int c, int d, int e)
{
...
void Output_footer(int a, int Cumi_Steps, int Cumi_Step_units)
{
	if (a==1)
	{
